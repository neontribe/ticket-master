...
function Ticket(data, type) {
    data = data || {
        name: "nothing_here",
        id: "nothing_here",
        content: "nothing_here",
        actions: "nothing_here",
        comments: "nothing_here",
        attachments: "nothing_here"
    };

    if(type == "jira") {
        this.name = utils.truncate(data.fields.description.replace(/\//g, "-").toLowerCase(), 30, true);
        this.uneditedName = data.fields.description;

        this.id = data.id;
        this.jiraId = data.key;
        this.parentBoard = data.fields.project.name;
        ...
        this.content = data.fields.description;
...
Ticket.prototype.setDiff = function(new_ver, old_ver) {
  //  this.diff = jsdiff.diffWords(new_ver, old_ver);
    this.diff = jsdiff.diffJson(JSON.parse(new_ver), JSON.parse(old_ver));
