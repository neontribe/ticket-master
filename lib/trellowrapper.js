...
//**
/*
                If isn't first time run:
                    -> Check if ticket is new (it exists in dump directory)
                        -> True:
                            -> Check if ticket has NOJIRA flag
                                -> True:
                                    Make ticket file with NOJIRA flag := true, set `changed` flag to false
                                -> False:
                                    Make ticket file with NOJIRA flag := false, set `changed` flag to false
                        -> False:
                            -> Has the ticket changed?
                                -> True:
                                   
                                        ->True:
                                            Generate diff, show user for confirmation to overwrite
                                                -> True
                                                    Make changes to locally stored ticket
                                                -> False
                                                    Don't change locally stored ticket
                                        ->False:
                                            Continue;
                                -> False:
                                    Continue;
    ...
    (function boardColTick(i) {
        ...
        _this.getTickets(ticket, col_ids[i][3], function(err, data) {
            (function ticketTick(j) {
                ...
                //TODO: Set nochanges and nojira flags based on specific comment/heading of ticket
                ...
                if (file) {
                    //If they're the same
                    if (JSON.stringify(newTicket) == file.toString()) {
                        ...
                        paused = true;
                        //Gen diff, ask for confirmation
                        var diff = jsdiff.diffWords(file.toString(), JSON.stringify(newTicket));
                        ...
                        rl.question("Are you happy with these changes? Y/n", function(ans) {
                            paused = false;
                            ...
                            if (ans.toLowerCase() == "y") {
                                //On ticket verify **
                                ticket_ids.push([col_ids[i], newTicket.name, newTicket.id]);
                                _this.parent.dumpJSON(ticketDir, JSON.stringify(newTicket));
                                ...
                                console.log("Skipping the update of this ticket.");
